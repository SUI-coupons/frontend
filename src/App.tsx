      ...
      >
        <div className="flex gap-4 items-center">
          ...
          <div className="flex gap-4">
            ...
            <Link to={`/my-listings/${currentAccount ? currentAccount.address : ''}`}>My Listings</Link>
        ...
        <Box>
          <ConnectModal
            trigger={
              <button className="bg-[#4DA2FF] p-2 rounded-md" disabled={!!currentAccount}> {currentAccount ? currentAccount.address.slice(0,5) + '...' + currentAccount.address.slice(currentAccount.address.length - 6, currentAccount.address.length - 1) : 'Connect'}</button>
            }
            open={open}
